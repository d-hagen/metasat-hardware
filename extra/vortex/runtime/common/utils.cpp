...
extern int vx_dump_perf(vx_device_h hdevice, FILE* stream) {
  ...
  for (unsigned core_id = 0; core_id < num_cores; ++core_id) {    
    uint64_t mpm_mem_addr = IO_MPM_ADDR + core_id * staging_buf.size();    
    ...
    case VX_DCR_MPM_CLASS_MEM: {      
      if (lmem_enable) {
        ...
        int lmem_bank_utilization = calcUtilization(lmem_reads + lmem_writes, lmem_bank_stalls);
...
extern int vx_perf_counter(vx_device_h hdevice, int counter, int core_id, uint64_t* value) {
  ...
  for (i = 0; i < num_cores; ++i) {
    uint64_t mpm_mem_addr = IO_MPM_ADDR + i * staging_buf.size();    
    ret = vx_copy_from_dev(hdevice, staging_buf.data(), mpm_mem_addr, staging_buf.size());
    if (ret != 0)
      return ret;
