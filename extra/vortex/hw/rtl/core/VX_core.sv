...
`endif
    ...
    //Debug signals
    reg  [`XLEN-1:0] debug_fetch [2];
    reg  [`XLEN-1:0] debug_decode;
    reg  [`XLEN-1:0] debug_issue [`ISSUE_WIDTH];
    reg  [`XLEN-1:0] debug_commit[`ISSUE_WIDTH];
    wire [`XLEN-1:0] debug_stall [`ISSUE_WIDTH];
...
`ifdef EXT_F_ENABLE
                end else if (fpu_dispatch_if[i].valid) begin
                    debug_issue[i] <= fpu_dispatch_if[i].data.PC;
`endif
...
`ifdef EXT_F_ENABLE
                end else if (fpu_commit_if[i].valid) begin
                    debug_commit[i] <= fpu_commit_if[i].data.PC;
`endif
