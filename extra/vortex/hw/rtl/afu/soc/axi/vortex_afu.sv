...
`ifndef SYNTHESIS
	// ===== Vortex AXI Monitor (simulation only) =====
	// Monitors GPU-side AXI transactions (post-reset only).
	// Logs first 32 AR and AW addresses, then periodic counts every 500k cycles.

	localparam VXM_ADDR_LOG = 32;

	reg [31:0]        vxm_ar_cnt, vxm_aw_cnt, vxm_w_cnt, vxm_r_cnt, vxm_b_cnt;
	...
	always @(posedge clk) begin
		if (vx_reset) begin
			...
			// W / R / B
			if (m_axi_mem_wvalid  && m_axi_mem_wready)  vxm_w_cnt    <= vxm_w_cnt + 1;
			if (m_axi_mem_rvalid  && m_axi_mem_rready) begin
				vxm_r_cnt    <= vxm_r_cnt + 1;
				vxm_outst_rd <= vxm_outst_rd - 1;
			end
			if (m_axi_mem_bvalid  && m_axi_mem_bready) begin
				vxm_b_cnt    <= vxm_b_cnt + 1;
				vxm_outst_wr <= vxm_outst_wr - 1;
			end
			// Periodic summary
			if (vxm_active) begin
				vxm_period <= vxm_period + 1;
				if (vxm_period == 32'd500_000) begin
					vxm_period <= 0;
					$display("[%0t] VX AXI: AR=%0d AW=%0d W=%0d R=%0d B=%0d outst rd=%0d wr=%0d",
					         $time, vxm_ar_cnt, vxm_aw_cnt, vxm_w_cnt,
					         vxm_r_cnt, vxm_b_cnt, vxm_outst_rd, vxm_outst_wr);
				end
			end
		end
	end
