...
module VX_afu_ctrl #(
	...
	// ================================================================
	// SIM-ONLY: AFU diagnostics (eval-debug)
	//
	// Piece 1 — AFU state-machine watchdog: logs STATE_RUN entry/exit
	// and every transition of vx_running / vx_busy / vx_busy_wait while
	// in STATE_RUN. Fires a one-shot stall classifier if STATE_RUN is
	// held for >100k cycles without resolving.
	//
	// Piece 2 — DCR write trace: prints every DCR write (the host
	// writes 5 of these during vx_start; if any is wrong, Vortex will
	// fetch the wrong startup address).
	//
	// $display calls are stripped by synthesis; counter regs become
	// dead logic and are trimmed. Safe to leave unconditional.
	// ================================================================
