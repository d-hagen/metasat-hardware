module VX_commit import VX_gpu_pkg::*, VX_trace_pkg::*; #(
    ...
    input wire              clk,
    input wire              reset,
    VX_commit_if.slave      commit_if [(3 + 0) * (((2 / 8) != 0) ? (2 / 8) : 1)],
    VX_writeback_if.master  writeback_if  [(((2 / 8) != 0) ? (2 / 8) : 1)],
    VX_commit_csr_if.master commit_csr_if,
    VX_commit_sched_if.master commit_sched_if
);
    localparam DATAW = 1 + ((($clog2(2)) != 0) ? ($clog2(2)) : 1) + 2 + (32-1) + 1 + $clog2(32) + 2 * 32 + 1 + 1 + 1;
    localparam COMMIT_SIZEW = $clog2(2 + 1);
    localparam COMMIT_ALL_SIZEW = COMMIT_SIZEW + (((2 / 8) != 0) ? (2 / 8) : 1) - 1;
    VX_commit_if commit_arb_if[(((2 / 8) != 0) ? (2 / 8) : 1)]();
    wire [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0] per_issue_commit_fire;
    wire [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0][((($clog2(2)) != 0) ? ($clog2(2)) : 1)-1:0] per_issue_commit_wid;
    wire [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0][2-1:0] per_issue_commit_tmask;
    wire [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0] per_issue_commit_eop;
    for (genvar i = 0; i < (((2 / 8) != 0) ? (2 / 8) : 1); ++i) begin
        ...
        for (genvar j = 0; j < (3 + 0); ++j) begin
            assign valid_in[j] = commit_if[j * (((2 / 8) != 0) ? (2 / 8) : 1) + i].valid;
            assign data_in[j]  = commit_if[j * (((2 / 8) != 0) ? (2 / 8) : 1) + i].data;
            assign commit_if[j * (((2 / 8) != 0) ? (2 / 8) : 1) + i].ready = ready_in[j];
        end
    ...
    VX_reset_relay #(.N(1), .MAX_FANOUT(0)) __arb_reset ( 
        ...
        assign per_issue_commit_tmask[i]= {2{per_issue_commit_fire[i]}} & commit_arb_if[i].data.tmask;
        assign per_issue_commit_wid[i]  = commit_arb_if[i].data.wid;
        assign per_issue_commit_eop[i]  = commit_arb_if[i].data.eop;
    end
    wire [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0][COMMIT_SIZEW-1:0] commit_size, commit_size_r;
    ...
    VX_pipe_register #(
        .DATAW  (1 + (((2 / 8) != 0) ? (2 / 8) : 1) * COMMIT_SIZEW),
        .RESETW (1)
    ) commit_size_reg1 (
        .clk      (clk),
        .reset    (reset),
