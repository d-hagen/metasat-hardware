module VX_mem_adapter #(
    ...
    localparam D = (((DST_LDATAW - SRC_LDATAW) < 0) ? (-(DST_LDATAW - SRC_LDATAW)) : (DST_LDATAW - SRC_LDATAW));;
    localparam P = 2**D;
    ...
    VX_elastic_buffer #(
        ...
        .SIZE     ((((RSP_OUT_BUF) < (2)) ? (RSP_OUT_BUF) : (2))),
        .OUT_REG  (((RSP_OUT_BUF < 2) ? RSP_OUT_BUF : (RSP_OUT_BUF - 2)))
    ) rsp_in_buf (
        .clk       (clk),
        .reset     (reset),
