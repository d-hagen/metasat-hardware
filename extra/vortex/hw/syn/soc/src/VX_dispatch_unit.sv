module VX_dispatch_unit import VX_gpu_pkg::*; #(
    ...
    localparam IN_DATAW     = 1 + ISSUE_WIS_W + 4 + 4 + $bits(op_args_t) + 1 + (32-1) + $clog2((2 * 32)) + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + (3 * 4 * 32);
    localparam OUT_DATAW    = 1 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + 4 + $bits(op_args_t) + 1 + (32-1) + $clog2((2 * 32)) + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + (3 * NUM_LANES * 32) + PID_WIDTH + 1 + 1;
