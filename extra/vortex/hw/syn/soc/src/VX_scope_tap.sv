module VX_scope_tap #(     
    parameter SCOPE_ID  = 0,     
    parameter SCOPE_IDW = 8,     
    parameter TRIGGERW  = 0,     
    parameter PROBEW    = 0,     
    parameter SIZE      = 256,   
    parameter IDLE_CTRW = 16     
) ( 
    input wire clk,
    input wire reset,
    input wire start,
    ...
    localparam TX_DATA_BITS = (((TX_DATAW) > 1) ? $clog2(TX_DATAW) : 1);
    localparam DATAW        = PROBEW + TRIGGERW;    
    localparam DATA_BITS    = (((DATAW) > 1) ? $clog2(DATAW) : 1);
    ...
    (* rw_addr_collision = "no" *) reg [DATAW-1:0] data_store [SIZE-1:0];
    (* rw_addr_collision = "no" *) reg [IDLE_CTRW-1:0] delta_store [SIZE-1:0];
