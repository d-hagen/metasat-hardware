module VX_operands import VX_gpu_pkg::*; #(
    ...
    VX_ibuffer_if.slave     scoreboard_if [(((4) < (4)) ? (4) : (4))],
    VX_operands_if.master   operands_if [(((4) < (4)) ? (4) : (4))]
);
    localparam DATAW = 1 + ISSUE_WIS_W + 4 + 32 + 1 + $clog2((3 + 0)) + 4 + 3 + 1 + 1 + 32 + $clog2(32);
    ...
    for (genvar i = 0; i < (((4) < (4)) ? (4) : (4)); ++i) begin
        wire [4-1:0][32-1:0] gpr_rd_data;
        reg [$clog2(32)-1:0] gpr_rd_rid, gpr_rd_rid_n;
        reg [ISSUE_WIS_W-1:0] gpr_rd_wis, gpr_rd_wis_n;
        reg [ISSUE_RATIO-1:0][4-1:0][32-1:0] cache_data, cache_data_n;
        reg [ISSUE_RATIO-1:0][$clog2(32)-1:0] cache_reg, cache_reg_n;
        reg [ISSUE_RATIO-1:0][4-1:0] cache_tmask, cache_tmask_n;
        reg [ISSUE_RATIO-1:0] cache_eop, cache_eop_n;
        reg [4-1:0][32-1:0] rs1_data, rs1_data_n;
        reg [4-1:0][32-1:0] rs2_data, rs2_data_n;
        reg [4-1:0][32-1:0] rs3_data, rs3_data_n;   
        reg [STATE_BITS-1:0] state, state_n;
        reg [$clog2(32)-1:0] rs2, rs2_n;
        reg [$clog2(32)-1:0] rs3, rs3_n;
