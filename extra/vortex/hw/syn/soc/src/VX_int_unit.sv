module VX_int_unit #(
    ...
    wire [NUM_LANES-1:0][32-1:0] shr_result;
    ...
    wire                    is_br_op = execute_if.data.op_mod[0];
    ...
    wire [NUM_LANES-1:0][32-1:0] alu_in1_PC  = execute_if.data.use_PC ? {NUM_LANES{execute_if.data.PC}} : alu_in1;
    wire [NUM_LANES-1:0][32-1:0] alu_in2_imm = execute_if.data.use_imm ? {NUM_LANES{execute_if.data.imm}} : alu_in2;
    wire [NUM_LANES-1:0][32-1:0] alu_in2_br  = (execute_if.data.use_imm && ~is_br_op) ? {NUM_LANES{execute_if.data.imm}} : alu_in2;
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin    
        ...
        assign shr_result[i] = 32'($signed(shr_in1) >>> alu_in2_imm[i][SHIFT_IMM_BITS-1:0]);
    ...
    for (genvar i = 0; i < NUM_LANES; ++i) begin
        ...
        always @(*) begin
            case ({is_alu_w, op_class})                        
                ...
                3'b010: alu_result[i] = shr_result[i];       
    ...
    VX_elastic_buffer #(
        .DATAW (1 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + $clog2(32) + 1 + PID_WIDTH + 1 + 1 + (NUM_LANES * 32) + 32 + 32 + 1 + 4 + LANE_WIDTH)
    ) rsp_buf (
        .clk      (clk),
        .reset    (reset),
    ...
    wire [32-1:0] br_dest = is_br_static ? br_result : (PC_r + imm_r);
    wire [((($clog2(4)) != 0) ? ($clog2(4)) : 1)-1:0] br_wid;
    if (((((((4) < (4)) ? (4) : (4)) / 1) != 0) ? ((((4) < (4)) ? (4) : (4)) / 1) : 1) != 1) begin 
        if (((((((4) < (4)) ? (4) : (4)) / 1) != 0) ? ((((4) < (4)) ? (4) : (4)) / 1) : 1) != 4) begin 
            assign br_wid = {commit_if.data.wid[((($clog2(4)) != 0) ? ($clog2(4)) : 1)-1:$clog2(((((((4) < (4)) ? (4) : (4)) / 1) != 0) ? ((((4) < (4)) ? (4) : (4)) / 1) : 1))], $clog2(((((((4) < (4)) ? (4) : (4)) / 1) != 0) ? ((((4) < (4)) ? (4) : (4)) / 1) : 1))'(BLOCK_IDX)}; 
        end else begin 
    ...
    VX_pipe_register #(
        .DATAW (1 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + 1 + 32)
    ) branch_reg (
        .clk      (clk),
        .reset    (reset),
        .enable   (1'b1),
