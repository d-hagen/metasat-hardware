module VX_lsu_slice import VX_gpu_pkg::*, VX_trace_pkg::*; #(
    ...
    localparam RSP_ARB_DATAW= 23 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + (32-1) + $clog2(32) + 1 + NUM_LANES * 32 + PID_WIDTH + 1 + 1;
    ...
    localparam TAG_WIDTH = 23 + TAG_ID_WIDTH;
    VX_commit_if #(
        .NUM_LANES (NUM_LANES)
    ) commit_rsp_if();
    VX_commit_if #(
...
;
    ...
    if (PID_BITS != 0) begin
        reg [(2 * (4 / 4))-1:0][PID_BITS:0] pkt_ctr;
        reg [(2 * (4 / 4))-1:0] pkt_sop, pkt_eop;
    ...
    VX_elastic_buffer #(
        .DATAW (23 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + (32-1) + 1 + $clog2(32) + (NUM_LANES * 32) + PID_WIDTH + 1 + 1),
        .SIZE  (2)
    ) rsp_buf (
        .clk       (clk),
        .reset     (reset),
    ...
    VX_elastic_buffer #(
        .DATAW (23 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + (32-1) + PID_WIDTH + 1 + 1),
        .SIZE  (2)
    ) no_rsp_buf (
        .clk       (clk),
        .reset     (reset),
