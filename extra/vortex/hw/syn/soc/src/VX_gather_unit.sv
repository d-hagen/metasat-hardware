module VX_gather_unit import VX_gpu_pkg::*; #(
    ...
    VX_commit_if.master commit_out_if [(((2 / 8) != 0) ? (2 / 8) : 1)]
);
    ...
    localparam DATAW        = 1 + ((($clog2(2)) != 0) ? ($clog2(2)) : 1) + NUM_LANES + (32-1) + 1 + $clog2(32) + NUM_LANES * 32 + PID_WIDTH + 1 + 1;
    localparam DATA_WIS_OFF = DATAW - (1 + ((($clog2(2)) != 0) ? ($clog2(2)) : 1));
    ...
    for (genvar i = 0; i < BLOCK_SIZE; ++i) begin
        ...
        if (BLOCK_SIZE != (((2 / 8) != 0) ? (2 / 8) : 1)) begin
    ...
    reg [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0] commit_out_valid;
    reg [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0][DATAW-1:0] commit_out_data;
    wire [(((2 / 8) != 0) ? (2 / 8) : 1)-1:0] commit_out_ready;
    always @(*) begin
        commit_out_valid = '0;
    ...
    for (genvar i = 0; i < (((2 / 8) != 0) ? (2 / 8) : 1); ++i) begin
        ...
        logic [2-1:0] commit_tmask_r;
        logic [2-1:0][32-1:0] commit_data_r;
        if (PID_BITS != 0) begin
            always @(*) begin
                commit_tmask_r = '0;
                commit_data_r  = 'x;
