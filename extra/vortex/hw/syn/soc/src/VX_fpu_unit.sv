module VX_fpu_unit import VX_fpu_pkg::*; #(
    ...
    VX_reset_relay #(.N(1), .MAX_FANOUT((((BLOCK_SIZE > 1)) ? 0 : -1))) __block_reset ( 
        ...
        VX_index_buffer #(
            .DATAW  (23 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + (32-1) + $clog2(32) + PID_WIDTH + 1 + 1),
            .SIZE   ((2 * (4 / 4)))
        ) tag_store (
            .clk          (clk),
            .reset        (block_reset),
    ...
    if ((((4 / 8) != 0) ? (4 / 8) : 1) != 1) begin 
        ...
        VX_elastic_buffer #(
            .DATAW (23 + ((($clog2(4)) != 0) ? ($clog2(4)) : 1) + NUM_LANES + (32-1) + $clog2(32) + (NUM_LANES * 32) + PID_WIDTH + 1 + 1),
            .SIZE  (0)
        ) rsp_buf (
            .clk       (clk),
            .reset     (block_reset),
