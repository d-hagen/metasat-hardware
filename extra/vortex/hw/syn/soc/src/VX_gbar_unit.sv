module VX_gbar_unit #(    
    ...
    reg [((($clog2(4)) != 0) ? ($clog2(4)) : 1)-1:0][8-1:0] barrier_masks;
    wire [$clog2(8+1)-1:0] active_barrier_count;
    wire [8-1:0] curr_barrier_mask = barrier_masks[gbar_bus_if.req_id];
    VX_popcount #( 
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (gbar_bus_if.req_valid) begin
                if (active_barrier_count[((($clog2(8)) != 0) ? ($clog2(8)) : 1)-1:0] == gbar_bus_if.req_size_m1) begin
