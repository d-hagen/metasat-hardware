module VX_gbar_unit #(    
    ...
    reg [((($clog2(4)) != 0) ? ($clog2(4)) : 1)-1:0][2-1:0] barrier_masks;
    wire [$clog2(2+1)-1:0] active_barrier_count;
    wire [2-1:0] curr_barrier_mask = barrier_masks[gbar_bus_if.req_id];
    VX_popcount #( 
        .N ($bits(curr_barrier_mask)), 
        .MODEL (1) 
    ) __active_barrier_count__ ( 
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (gbar_bus_if.req_valid) begin
                if (active_barrier_count[((($clog2(2)) != 0) ? ($clog2(2)) : 1)-1:0] == gbar_bus_if.req_size_m1) begin
