module VX_cache_bypass #(
    ...
    localparam MUX_DATAW        = 1 + WORD_SIZE + CORE_ADDR_WIDTH + (2 + 1) + CORE_DATA_WIDTH + CORE_TAG_WIDTH;
    ...
    wire [(((REQ_SEL_BITS) != 0) ? (REQ_SEL_BITS) : 1)-1:0] core_req_nc_idx;
    ...
    for (genvar i = 0; i < NUM_REQS; ++i) begin
        if (PASSTHRU != 0) begin
            ...
            assign core_req_nc_idxs[i] = core_bus_in_if[i].req_data.atype[1];
        end else begin
    ...
    wire [(32-$clog2(LINE_SIZE))-1:0] mem_req_out_addr;
    wire [(2 + 1)-1:0] mem_req_out_atype;
    wire [(8 * LINE_SIZE)-1:0]   mem_req_out_data;
    ...
    wire [(2 + 1)-1:0] core_req_nc_sel_atype;
    ...
    VX_elastic_buffer #(
        .DATAW   (1 + LINE_SIZE + (32-$clog2(LINE_SIZE)) + (2 + 1) + (8 * LINE_SIZE) + MEM_TAG_OUT_WIDTH),
        .SIZE    ((!DIRECT_PASSTHRU) ? (((MEM_OUT_BUF) < (2)) ? (MEM_OUT_BUF) : (2)) : 0),
    ...
    wire [NUM_REQS-1:0][(8 * WORD_SIZE)-1:0] core_rsp_in_data;
    ...
    wire [(((REQ_SEL_BITS) != 0) ? (REQ_SEL_BITS) : 1)-1:0] rsp_idx;
    if (NUM_REQS > 1) begin
    ...
    for (genvar i = 0; i < NUM_REQS; ++i) begin
        VX_elastic_buffer #(
            .DATAW   ((8 * WORD_SIZE) + CORE_TAG_WIDTH),
            .SIZE    ((!DIRECT_PASSTHRU) ? (((CORE_OUT_BUF) < (2)) ? (CORE_OUT_BUF) : (2)) : 0),
